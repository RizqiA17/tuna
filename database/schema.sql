...
INSERT INTO
    game_settings (setting_key, setting_value, description)
VALUES
    (
        'answer_time_limit',
        '900',
        'Time limit in seconds for teams to answer each scenario (default: 15 minutes)'
    );
...
CREATE TABLE game_containers (
    id INT UNSIGNED NOT NULL AUTO_INCREMENT,
    name VARCHAR(255) NOT NULL,
    status ENUM('menunggu', 'mulai', 'selesai') NOT NULL DEFAULT 'menunggu',
...
ALTER TABLE teams 
    ADD COLUMN game_container_id INT UNSIGNED NULL AFTER total_score,
    ADD FOREIGN KEY (game_container_id) REFERENCES game_containers(id) ON DELETE CASCADE 

ALTER TABLE
    `tuna_adventure`.`teams` DROP INDEX `idx_teams_name`,
ADD
    INDEX `idx_teams_name` (`name`, `game_container_id`) USING BTREE;

ALTER TABLE
    `teams` DROP INDEX `name`;