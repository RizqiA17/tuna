...
class AdminPanel {
  ...
  updateTeamCompleted() {
    ...
    if (teamCompleted) {
      teamCompleted.textContent = `${this.teamsCompletedCurrentStep.size}/${this.connectedTeams.size}`;
    }
  }
  ...
  updatePosition() {
    ...
    if (positionElement) {
      positionElement.textContent = this.currentStep;
    }
  }
  ...
  updateGameControlButtons() {
    ...
    if (nextBtn) {
      if (this.gameState === "running") {
        ...
        console.log(
          `🎯 Step completion: ${this.teamsCompletedCurrentStep.size}/${this.connectedTeams.size} teams completed`
        );
      } else {
  ...
  updateOverview() {
    ...
    document.getElementById("totalTeams").textContent =
      this.statsData.totalTeams || 0;
    document.getElementById("activeTeams").textContent =
      this.statsData.activeTeams || 0;
    document.getElementById("completedTeams").textContent =
      this.statsData.completedTeams || 0;
    document.getElementById("averageScore").textContent = this.statsData
      .averageScore
      ? Math.round(this.statsData.averageScore)
      : 0;
  ...
  updateGameSettingsUI() {
    ...
    if (this.gameSettings && this.gameSettings.answer_time_limit) {
      const timeLimitSeconds = parseInt(
        this.gameSettings.answer_time_limit.value
      );
